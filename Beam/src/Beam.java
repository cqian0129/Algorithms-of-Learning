...
//using beam heuristic

public class Beam {
    private final int BEAMSIZE = 2;
    ...
    private class Path
    {
        ...
        private int last()
        {
            return list.getLast();                  //return last veretx
        }
    ...
    public Beam(int vertices, int[][] edges)
    {
            ...
            else                                    //if path is not complete
            {
                ...
                for (int i = 0; i < children.size() && i < BEAMSIZE; i++)
                    list.addFirst(children.get(i));
                                                    //add at most beam size children
                ...
                if (i == initial)                   //if vertex is initial vertex
                {
                    if (path.size() == size)        //if path has size vertices
                    {
                        ...
                        child.add(i, matrix[lastVertex][i]);
                                                            //add vertex to path
                        children.addLast(child);            //add extended path ot children list
                    }
                }
    ...
    //Method decides whether a paht is complete
    boolean complete(Path path)
    {
        return path.size() == size + 1;                     //check paht has size+1
    }                                                       //number of vertices
}
