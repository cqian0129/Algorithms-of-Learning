...
public class NearestNeighbor {
    ...
    public void classifyData (String testFile, String classifiedFile) throws IOException
    {
        ...
        outFile.println("classification of records when k is " + numberNeighbors + " and majority rule is " + majorityRule);
    ...
    //Method finds class of given attributes
    private int classify_leaveOneOut(double[] attributes, int a)
    {
        double[] distance = new double[numberRecords-1];
        int[] id = new int[numberRecords-1];
        int index = 0;

        for (int i = 0; i < numberRecords-1 && index < numberRecords; i++){
            if (i == a) {
                index++;
            }
            distance[i] = distance(attributes, records.get(index).attributes);
            id[i] = index;
            index++;
        }

        //find the nearest neighbors
        nearestNeighbor_leaveOneOut(distance, id);
    ...
    //Method finds the nearest neighbors
    private void nearestNeighbor_leaveOneOut(double[] distance, int[] id)
    {
        //sort the records by their distances and choose the closest neighbors
        for (int i = 0; i < numberNeighbors; i++)
    ...
    //Method find training error
    public void trainingErr (String trainingFile) throws IOException
    {
        Scanner inFile = new Scanner(new File(trainingFile));
